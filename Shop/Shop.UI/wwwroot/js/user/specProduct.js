    ...
    methods: {
        ...
        saleEnable(sale, value, qty) {
            if (sale == 0) {
                ...
                const costSale = (value).replace(",", ".") * (sale / 100);
        ...
        async addOneToWish() {
            try {
                await http.post("/Wish/AddOneWish/" + option.getAttribute('data-id'));
                this.updateWish();
        ...
        async addOneToCompare() {
            ...
            try {
                await http.post("/Compare/AddOneCompare/" + (option.getAttribute('data-id') + "," + this.selectedProduct.specification[0].id));
        ...
        async addReview(product) {
            ...
            try {
                ...
                if (this.selectedProduct.review.length > 0) {
                    let i = 0;
                    for (let b = 0; b < this.selectedProduct.review.length; b++) {
            ...
            try {
                await http.put('/products', formData);
            } catch (err) {
                console.log(err);
            }
            this.loading = false;
        ...
        getSpecs() {
            window.addEventListener("load", function () {
                ...
                const result = Object.values(this.selectedProduct.specification[0]);
                const info = [
                    'id',
                ...
                for (let i = 1; i < result.length; i++) {
                    if (result[i] != " ") {
                        ...
                        const caption = row.insertCell(0);
